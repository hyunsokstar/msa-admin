...
export async function GET(req: NextRequest) {
    ...
    const testItemId = pathname.split("/").at(-2); // /api/test-items/[id]/comments → [id] 추출

    if (!testItemId) {
        return new Response(JSON.stringify({ error: "testItemId가 제공되지 않았습니다." }), {
            status: 400,
        });
    }
...
export async function POST(req: NextRequest) {
    const supabase = getSupabaseService();
    const pathname = req.nextUrl.pathname;
    const testItemId = pathname.split("/").at(-2); // [id] 추출
