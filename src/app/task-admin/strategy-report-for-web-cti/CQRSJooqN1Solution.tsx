...
const SimpleN1SolutionGuide = () => {
  const [activeSection, setActiveSection] = useState('basic');

  const SolutionCard = ({ level, title, description, pros, cons, needsDataLoader }) => (
    <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
      ...
      <div className={`p-3 rounded-lg ${needsDataLoader ? 'bg-yellow-50 border border-yellow-200' : 'bg-blue-50 border border-blue-200'}`}>
  ...
  const CodeExample = ({ title, code, language = 'java' }) => (
    ...
    <div className="space-y-8">
      <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg">
        ...
        <div className="text-lg text-gray-700 space-y-2">
          <p><strong>JPA + jOOQ만으로도 N+1 문제 상당 부분 해결됩니다!</strong></p>
          <p>DataLoader는 <span className="text-blue-600 font-medium">GraphQL에서 완벽한 최적화</span>를 원할 때 추가하면 좋습니다.</p>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        ...
        <ul className="space-y-2 text-yellow-700">
          <li className="flex items-start gap-2">
            ...
            <span><strong>점진적 적용:</strong> 먼저 JPA Join Fetch부터 시작해서 필요시 jOOQ 추가</span>
          </li>
          <li className="flex items-start gap-2">
            <ArrowRight className="w-4 h-4 mt-0.5 text-yellow-600" />
            <span><strong>측정 우선:</strong> 실제 N+1 문제가 있는지 먼저 확인 (jOOQ DiagnosticsConnection 활용)</span>
          </li>
          <li className="flex items-start gap-2">
            <ArrowRight className="w-4 h-4 mt-0.5 text-yellow-600" />
            <span><strong>역할 분담:</strong> 간단한 CRUD는 JPA, 복잡한 조회는 jOOQ</span>
          </li>
          <li className="flex items-start gap-2">
            <ArrowRight className="w-4 h-4 mt-0.5 text-yellow-600" />
            <span><strong>DataLoader:</strong> GraphQL + 높은 트래픽 환경에서만 고려</span>
          </li>
        </ul>
      </div>
    </div>
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="bg-gray-100 border-b border-gray-200">
          <div className="flex space-x-1 p-1">
            <TabButton id="basic" label="기본 이해" isActive={activeSection === 'basic'} />
            <TabButton id="comparison" label="상세 비교" isActive={activeSection === 'comparison'} />
            <TabButton id="examples" label="코드 예시" isActive={activeSection === 'examples'} />
          </div>
        </div>
