// components/NextGenerationArchitectureWithRedisScaling.tsx
"use client";

import React from "react";

const NextGenerationArchitectureWithRedisScaling: React.FC = () => {
  return (
    <div className="w-full p-4 bg-gray-50 space-y-8">
      {/*** 1. 차세대 프로젝트 아키텍처 다이어그램 ***/}
      <svg
        viewBox="0 0 1400 900"
        className="w-full h-auto border rounded-lg shadow-lg bg-white"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          {/* Gradients */}
          <linearGradient id="electronGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#47848f" stopOpacity="1" />
            <stop offset="100%" stopColor="#2c5f6a" stopOpacity="1" />
          </linearGradient>

          <linearGradient id="gatewayGreenGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#4ade80" stopOpacity="1" />
            <stop offset="100%" stopColor="#16a34a" stopOpacity="1" />
          </linearGradient>

          <linearGradient id="blueServiceGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#336791" stopOpacity="1" />
            <stop offset="100%" stopColor="#254670" stopOpacity="1" />
          </linearGradient>

          <linearGradient id="wsServiceGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#fcd34d" stopOpacity="1" />
            <stop offset="100%" stopColor="#fbbf24" stopOpacity="1" />
          </linearGradient>

          <linearGradient id="redisGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#DC382D" stopOpacity="1" />
            <stop offset="100%" stopColor="#a82920" stopOpacity="1" />
          </linearGradient>

          {/* DB Layer: Dark gray → near-black */}
          <linearGradient id="dbDarkGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#1f2937" stopOpacity="1" />
            <stop offset="100%" stopColor="#111827" stopOpacity="1" />
          </linearGradient>

          {/* Shadow Filter */}
          <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="3" dy="3" stdDeviation="3" floodColor="#000" floodOpacity="0.2" />
          </filter>

          {/* Arrow Markers */}
          <marker id="grpcArrow" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
            <polygon points="0 0, 10 3.5, 0 7" fill="#10b981" />
          </marker>
          <marker id="graphqlArrow" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
            <polygon points="0 0, 10 3.5, 0 7" fill="#e10098" />
          </marker>
          <marker id="redisArrow" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
            <polygon points="0 0, 10 3.5, 0 7" fill="#DC382D" />
          </marker>
          <marker id="wsArrow" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
            <polygon points="0 0, 10 3.5, 0 7" fill="#f59e0b" />
          </marker>
        </defs>

        {/* Background */}
        <rect width="1400" height="900" fill="#f8fafc" />

        {/* Title */}
        <text
          x="700"
          y="40"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="28"
          fontWeight="bold"
          fill="#1e293b"
        >
          차세대 프로젝트 아키텍처
        </text>
        <text
          x="700"
          y="65"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fill="#64748b"
        >
          Next Generation Project Architecture
        </text>

        {/* Client Layer */}
        <rect
          x="500"
          y="100"
          width="400"
          height="120"
          fill="url(#electronGrad)"
          stroke="#47848f"
          strokeWidth="3"
          rx="15"
          filter="url(#shadow)"
        />
        <text
          x="700"
          y="135"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="20"
          fontWeight="bold"
          fill="white"
        >
          Client (Electron + Next.js)
        </text>
        <text
          x="700"
          y="160"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#e2e8f0"
        >
          Desktop Application with Modern Web Technologies
        </text>
        <text
          x="700"
          y="180"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          GraphQL over HTTP/WebSocket
        </text>
        <text
          x="700"
          y="200"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          Real-time Updates &amp; Chat Interface
        </text>

        {/* API Gateway (WebSocket Server) */}
        <rect
          x="450"
          y="270"
          width="500"
          height="120"
          fill="url(#gatewayGreenGrad)"
          stroke="#16a34a"
          strokeWidth="3"
          rx="15"
          filter="url(#shadow)"
        />
        <text
          x="700"
          y="305"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="20"
          fontWeight="bold"
          fill="white"
        >
          API Gateway (NestJS GraphQL/WS)
        </text>
        <text
          x="700"
          y="330"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="white"
        >
          GraphQL Federation • gRPC Client • WebSocket Subscribers
        </text>
        <text
          x="700"
          y="350"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="white"
        >
          Authentication • Rate Limiting • Request Routing
        </text>
        <text
          x="700"
          y="370"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="white"
        >
          실시간 이벤트 브로드캐스팅 (WS + Redis Pub/Sub)
        </text>

        {/* Protocol Labels */}
        <rect x="580" y="240" width="80" height="20" fill="#e10098" rx="10" />
        <text
          x="620"
          y="253"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fontWeight="bold"
          fill="white"
        >
          GraphQL
        </text>
        <rect x="680" y="240" width="80" height="20" fill="#f59e0b" rx="10" />
        <text
          x="720"
          y="253"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fontWeight="bold"
          fill="white"
        >
          WebSocket
        </text>

        {/* Microservices Layer Title */}
        <text
          x="700"
          y="430"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="18"
          fontWeight="bold"
          fill="#475569"
        >
          Microservices Layer (gRPC 통신)
        </text>

        {/* User Service (blue) */}
        <rect
          x="100"
          y="470"
          width="200"
          height="140"
          fill="url(#blueServiceGrad)"
          stroke="#254670"
          strokeWidth="2"
          rx="12"
          filter="url(#shadow)"
        />
        <text
          x="200"
          y="500"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fontWeight="bold"
          fill="white"
        >
          User Service
        </text>
        <text
          x="200"
          y="520"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#e2e8f0"
        >
          NestJS
        </text>
        <text
          x="200"
          y="545"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 사용자 인증/인가
        </text>
        <text
          x="200"
          y="560"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 프로필 관리
        </text>
        <text
          x="200"
          y="575"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 권한 관리
        </text>
        <text
          x="200"
          y="590"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 조직 구조
        </text>

        {/* Chat Service (yellow: WebSocket) */}
        <rect
          x="350"
          y="470"
          width="200"
          height="140"
          fill="url(#wsServiceGrad)"
          stroke="#fbbf24"
          strokeWidth="2"
          rx="12"
          filter="url(#shadow)"
        />
        <text
          x="450"
          y="500"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fontWeight="bold"
          fill="#1e1e1e"
        >
          Chat Service (WebSocket)
        </text>
        <text
          x="450"
          y="520"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#1e1e1e"
        >
          Spring Boot
        </text>
        <text
          x="450"
          y="545"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • 채팅 메시지
        </text>
        <text
          x="450"
          y="560"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • 메시지 히스토리
        </text>
        <text
          x="450"
          y="575"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • 파일 첨부
        </text>
        <text
          x="450"
          y="590"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • 메시지 검색
        </text>

        {/* Call Service (yellow: WebSocket/Signaling) */}
        <rect
          x="600"
          y="470"
          width="200"
          height="140"
          fill="url(#wsServiceGrad)"
          stroke="#fbbf24"
          strokeWidth="2"
          rx="12"
          filter="url(#shadow)"
        />
        <text
          x="700"
          y="500"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fontWeight="bold"
          fill="#1e1e1e"
        >
          Call Service (WebSocket/Signaling)
        </text>
        <text
          x="700"
          y="520"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#1e1e1e"
        >
          Go (WebRTC)
        </text>
        <text
          x="700"
          y="545"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • VoIP/Media 관리
        </text>
        <text
          x="700"
          y="560"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • WebSocket 시그널링
        </text>
        <text
          x="700"
          y="575"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="#1e1e1e"
        >
          • WebRTC Peer Connection
        </text>

        {/* Notification Service (blue) */}
        <rect
          x="850"
          y="470"
          width="200"
          height="140"
          fill="url(#blueServiceGrad)"
          stroke="#254670"
          strokeWidth="2"
          rx="12"
          filter="url(#shadow)"
        />
        <text
          x="950"
          y="500"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fontWeight="bold"
          fill="white"
        >
          Notification Service
        </text>
        <text
          x="950"
          y="520"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#e2e8f0"
        >
          Spring Boot
        </text>
        <text
          x="950"
          y="545"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 실시간 알림
        </text>
        <text
          x="950"
          y="560"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 푸시 알림
        </text>
        <text
          x="950"
          y="575"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 이메일/SMS
        </text>
        <text
          x="950"
          y="590"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 알림 설정 관리
        </text>

        {/* Protocol Labels for gRPC */}
        <rect x="150" y="430" width="40" height="18" fill="#10b981" rx="9" />
        <text
          x="170"
          y="442"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="9"
          fontWeight="bold"
          fill="white"
        >
          gRPC
        </text>
        <rect x="400" y="430" width="40" height="18" fill="#10b981" rx="9" />
        <text
          x="420"
          y="442"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="9"
          fontWeight="bold"
          fill="white"
        >
          gRPC
        </text>
        <rect x="650" y="430" width="40" height="18" fill="#10b981" rx="9" />
        <text
          x="670"
          y="442"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="9"
          fontWeight="bold"
          fill="white"
        >
          gRPC
        </text>
        <rect x="900" y="430" width="40" height="18" fill="#10b981" rx="9" />
        <text
          x="920"
          y="442"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="9"
          fontWeight="bold"
          fill="white"
        >
          gRPC
        </text>

        {/* Redis Pub/Sub (Spring Boot) */}
        <rect
          x="1100"
          y="470"
          width="220"
          height="140"
          fill="url(#redisGrad)"
          stroke="#DC382D"
          strokeWidth="2"
          rx="12"
          filter="url(#shadow)"
        />
        <text
          x="1210"
          y="500"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="16"
          fontWeight="bold"
          fill="white"
        >
          Redis Pub/Sub Service
        </text>
        <text
          x="1210"
          y="520"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fill="#fecaca"
        >
          (Spring Boot)
        </text>
        <text
          x="1210"
          y="545"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 채팅 이벤트 전파
        </text>
        <text
          x="1210"
          y="560"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 통화 시그널 전파
        </text>
        <text
          x="1210"
          y="575"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 세션 상태 동기화
        </text>
        <text
          x="1210"
          y="590"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="11"
          fill="white"
        >
          • 실시간 알림 큐
        </text>

        {/* (1) API Gateway → User Service (gRPC) */}
        <path
          d="M 500 360 L 200 470"
          stroke="#10b981"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#grpcArrow)"
        />

        {/* (2) API Gateway → Chat Service (gRPC) */}
        <path
          d="M 580 360 L 450 470"
          stroke="#10b981"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#grpcArrow)"
        />

        {/* (3) API Gateway → Call Service (gRPC) */}
        <path
          d="M 700 360 L 700 470"
          stroke="#10b981"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#grpcArrow)"
        />

        {/* (4) API Gateway → Notification Service (gRPC) */}
        <path
          d="M 820 360 L 950 470"
          stroke="#10b981"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#grpcArrow)"
        />

        {/* Chat Service → Redis Pub/Sub */}
        <path
          d="M 450 600 L 1100 520"
          stroke="#DC382D"
          strokeWidth="1.5"
          fill="none"
          markerEnd="url(#redisArrow)"
        />

        {/* Call Service → Redis Pub/Sub */}
        <path
          d="M 700 600 L 1100 520"
          stroke="#DC382D"
          strokeWidth="1.5"
          fill="none"
          markerEnd="url(#redisArrow)"
        />

        {/* Notification Service → Redis Pub/Sub */}
        <path
          d="M 950 600 L 1210 520"
          stroke="#DC382D"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#redisArrow)"
        />

        {/* (A) Redis Pub/Sub → API Gateway (구독) */}
        <path
          d="M 1100 520 Q 900 350 700 320"
          stroke="#f59e0b"
          strokeWidth="2"
          fill="none"
          markerEnd="url(#wsArrow)"
          strokeDasharray="5,5"
        />

        {/* (B) API Gateway → Client (실제 WebSocket Push) */}
        <path
          d="M 700 300 L 700 220"
          stroke="#f59e0b"
          strokeWidth="3"
          fill="none"
          markerEnd="url(#wsArrow)"
        />

        {/* Services → Databases */}
        <path d="M 200 600 L 190 780" stroke="#1f2937" strokeWidth="2" fill="none" />
        <path d="M 450 600 L 410 780" stroke="#1f2937" strokeWidth="2" fill="none" />
        <path d="M 700 600 L 630 780" stroke="#1f2937" strokeWidth="2" fill="none" />
        <path d="M 950 600 L 1070 780" stroke="#1f2937" strokeWidth="1.5" fill="none" />

        {/* Cache connections (Call/Notification → Redis DB) */}
        <path d="M 700 600 L 820 780" stroke="#DC382D" strokeWidth="1.5" fill="none" />
        <path d="M 950 600 L 820 780" stroke="#DC382D" strokeWidth="1.5" fill="none" />

        <rect
          x="100"
          y="780"
          width="180"
          height="100"
          fill="url(#dbDarkGrad)"
          stroke="#111827"
          strokeWidth="2"
          rx="10"
          filter="url(#shadow)"
        />
        <text
          x="190"
          y="810"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fontWeight="bold"
          fill="white"
        >
          User DB
        </text>
        <text
          x="190"
          y="830"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          PostgreSQL
        </text>
        <text
          x="190"
          y="850"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#94a3b8"
        >
          사용자, 권한, 조직
        </text>

        <rect
          x="320"
          y="780"
          width="180"
          height="100"
          fill="url(#dbDarkGrad)"
          stroke="#111827"
          strokeWidth="2"
          rx="10"
          filter="url(#shadow)"
        />
        <text
          x="410"
          y="810"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fontWeight="bold"
          fill="white"
        >
          Message DB
        </text>
        <text
          x="410"
          y="830"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          MongoDB
        </text>
        <text
          x="410"
          y="850"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#94a3b8"
        >
          채팅, 메시지, 히스토리
        </text>

        <rect
          x="540"
          y="780"
          width="180"
          height="100"
          fill="url(#dbDarkGrad)"
          stroke="#111827"
          strokeWidth="2"
          rx="10"
          filter="url(#shadow)"
        />
        <text
          x="630"
          y="810"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fontWeight="bold"
          fill="white"
        >
          Call DB
        </text>
        <text
          x="630"
          y="830"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          PostgreSQL
        </text>
        <text
          x="630"
          y="850"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#94a3b8"
        >
          통화기록, CDR
        </text>

        <rect
          x="760"
          y="780"
          width="180"
          height="100"
          fill="url(#redisGrad)"
          stroke="#DC382D"
          strokeWidth="2"
          rx="10"
          filter="url(#shadow)"
        />
        <text
          x="850"
          y="810"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fontWeight="bold"
          fill="white"
        >
          Cache &amp; Session
        </text>
        <text
          x="850"
          y="830"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#fecaca"
        >
          Redis
        </text>
        <text
          x="850"
          y="850"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#fecaca"
        >
          세션, 캐시, 상태
        </text>

        <rect
          x="980"
          y="780"
          width="180"
          height="100"
          fill="url(#dbDarkGrad)"
          stroke="#111827"
          strokeWidth="2"
          rx="10"
          filter="url(#shadow)"
        />
        <text
          x="1070"
          y="810"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="14"
          fontWeight="bold"
          fill="white"
        >
          Analytics DB
        </text>
        <text
          x="1070"
          y="830"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fill="#cbd5e1"
        >
          ClickHouse
        </text>
        <text
          x="1070"
          y="850"
          textAnchor="middle"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#94a3b8"
        >
          로그, 분석, 통계
        </text>

        {/* Legend */}
        <rect x="50" y="20" width="300" height="60" fill="#f8fafc" stroke="#e2e8f0" strokeWidth="1" rx="5" />
        <text
          x="60"
          y="40"
          fontFamily="Arial, sans-serif"
          fontSize="12"
          fontWeight="bold"
          fill="#475569"
        >
          Legend:
        </text>
        <line x1="60" y1="50" x2="80" y2="50" stroke="#10b981" strokeWidth="2" markerEnd="url(#grpcArrow)" />
        <text
          x="85"
          y="55"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#475569"
        >
          gRPC
        </text>
        <line x1="60" y1="65" x2="80" y2="65" stroke="#e10098" strokeWidth="2" markerEnd="url(#graphqlArrow)" />
        <text
          x="85"
          y="70"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#475569"
        >
          GraphQL
        </text>
        <line x1="160" y1="50" x2="180" y2="50" stroke="#f59e0b" strokeWidth="2" markerEnd="url(#wsArrow)" />
        <text
          x="185"
          y="55"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#475569"
        >
          WebSocket
        </text>
        <line x1="160" y1="65" x2="180" y2="65" stroke="#DC382D" strokeWidth="2" markerEnd="url(#redisArrow)" />
        <text
          x="185"
          y="70"
          fontFamily="Arial, sans-serif"
          fontSize="10"
          fill="#475569"
        >
          Redis Pub/Sub
        </text>
      </svg>

    </div>
  );
};

export default NextGenerationArchitectureWithRedisScaling;
