...
interface TestTargetInfoProps {
    ...
    isArchived?: boolean; // isArchived 속성 추가
}

const TestTargetInfoComponent: React.FC<TestTargetInfoProps> = ({ testTarget, isArchived = false }) => {
    return (
        <div className="bg-white rounded-lg shadow-md p-4 mt-4">
            {/* 아카이브 표시 배지 */}
            {isArchived && (
                <div className="inline-block px-3 py-1 mb-3 bg-gray-100 text-gray-700 rounded-full text-sm">
                    아카이브됨
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-start md:gap-5">
                ...
                <div className="mb-4 md:mb-0 flex-shrink-0">
                    {testTarget.target_image_url ? (
                        <div className={`w-80 h-80 bg-gray-200 rounded-lg overflow-hidden shadow-md border ${isArchived ? 'border-gray-400 opacity-90' : 'border-gray-300'
                            }`}>
                            <ImageViewer
                        ...
                        <div className={`w-80 h-80 bg-gray-200 rounded-lg flex items-center justify-center shadow-md border ${isArchived ? 'border-gray-400 opacity-90' : 'border-gray-300'
                            }`}>
                ...
                <div className="flex-grow">
                    <h1 className="text-2xl font-bold mb-2">
                        {testTarget.name}
                        {isArchived && <span className="ml-2 text-sm font-normal text-gray-500">(아카이브)</span>}
                    </h1>
                    ...
                    <div className="mb-3 grid grid-cols-1 md:grid-cols-2 gap-3 w-full">
                        ...
                        {isArchived && (
                            <div className="bg-gray-50 p-2.5 rounded-md">
                                <span className="text-gray-500 font-medium">보관 상태:</span>
                                <span className="ml-2 text-gray-900">아카이브됨</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mt-3">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-4">
                            <div
                                className={`h-4 rounded-full ${isArchived ? 'opacity-70 ' : ''
                                    }${testTarget.completion_percentage < 30
                                        ? 'bg-red-500'
